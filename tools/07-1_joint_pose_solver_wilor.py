...
def reconstruct_left_hand_mesh(hand_data, frame_idx, mano_layer_left, device):
    import torch
...
def reconstruct_right_hand_mesh(hand_data, frame_idx, mano_layer_right, device):
    ...
    base_rot = torch.tensor(hand_data['right_hand_base_rot'][frame_idx]).to(device) if hand_data['right_hand_base_rot'].ndim == 3 else torch.eye(3).to(device)
...
class JointPoseSolver:
    ...
    def _mano_layer_forward_separate(self, pose_m, frame_idx=None):
        ...
        left_beta = torch.tensor(self._hand_data['left_hand_beta'][frame_idx]).to(device).float()
        right_beta = torch.tensor(self._hand_data['right_hand_beta'][frame_idx]).to(device).float()
        # verts_left, joints_left = self._mano_layer_left(left_pose, left_translation)
        # verts_left, joints_left = self._mano_layer_right(left_pose, left_translation)
        # verts_right, joints_right = self._mano_layer_right(right_pose, right_translation)
        ...
        # faces_left = self._mano_layer_left.f.detach().clone()
        # faces_left = self._mano_layer_left.f.detach().clone()
        faces_left = self._mano_layer_left.th_faces.detach().clone()
        # faces_right = self._mano_layer_right.f.detach().clone() + verts_left.shape[0]
    ...
    def render_optimized_poses(
        ...
        for i in range(num_frames):
            verts_left, faces_left = reconstruct_left_hand_mesh(self._hand_data, i, self._mano_layer_left, self._device)
