...
#     --tool_name mallet \
#     --tool_mesh /abs/path/to/models/mallet/textured_mesh.obj \
#     [--hand 1] [--optimize 1]
...
for required in SEQUENCE_FOLDER CALIBRATION_YAML TOOL_NAME TOOL_MESH; do
    if [[ -z "${!required}" ]]; then
...
if [[ "$SKIP_MASKS" == "1" ]] && ls "$TOOL_MASKS_DIR"/cam*_rgb/0000.npy >/dev/null 2>&1; then
    ...
    mkdir -p "$DINO_OUT"
    python "$HOCAP_ROOT/tools/dino_tool_segment.py" \
        --data_h5      "$H5_PATH" \
