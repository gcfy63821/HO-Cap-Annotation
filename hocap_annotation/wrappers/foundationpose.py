...
class FoundationPose:
    ...
    def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
        ...
        self.normals = F.normalize(
            torch.from_numpy(np.asarray(pcd.normals)).float().to(CFG.device), dim=-1
        )
        ...
        self.mesh_tensors = make_mesh_tensors(self.mesh, device=CFG.device)

        if symmetry_tfs is None:
            self.symmetry_tfs = torch.eye(4).float()[None].to(CFG.device)
        else:
            self.symmetry_tfs = torch.as_tensor(
                symmetry_tfs, device=CFG.device, dtype=torch.float
            )

        logging.info("reset done")

    def get_tf_to_centered_mesh(self):
        tf_to_center = torch.eye(4, dtype=torch.float, device=CFG.device)
        tf_to_center[:3, 3] = -torch.as_tensor(
            self.model_center, device=CFG.device, dtype=torch.float
        )
        return tf_to_center

    def to_device(self, s=CFG.device):
    ...
    def make_rotation_grid(self, min_n_views=40, inplane_step=60):
        ...
        self.rot_grid = torch.as_tensor(rot_grid, device=CFG.device, dtype=torch.float)
    ...
    def generate_random_pose_hypo(
        ...
        ob_in_cams[:, :3, 3] = torch.tensor(
            center, device=CFG.device, dtype=torch.float
        ).reshape(1, 3)
        return ob_in_cams
    ...
    def register(
        ...
        depth = erode_depth(depth, radius=2, device=CFG.device)
        depth = bilateral_filter_depth(depth, radius=2, device=CFG.device)
        ...
        poses = torch.as_tensor(poses, device=CFG.device, dtype=torch.float)
        poses[:, :3, 3] = torch.as_tensor(center.reshape(1, 3), device=CFG.device)
    ...
    def compute_add_err_to_gt_pose(self, poses):
        ...
        """
        return -torch.ones(len(poses), device=CFG.device, dtype=torch.float)
    ...
    def track_one(
        ...
        depth = torch.as_tensor(depth, device=CFG.device, dtype=torch.float)
        depth = erode_depth(depth, radius=2, device=CFG.device)
        depth = bilateral_filter_depth(depth, radius=2, device=CFG.device)
