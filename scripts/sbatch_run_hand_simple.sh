...
for TASK_DIR in "$VIDEOS_ROOT"/*/; do
    ...
    for EXP_DIR in "$TASK_DIR"*/; do
        ...
        # NB: we deliberately do NOT require masks. The inner pipeline
        # (batch_task_folder_hand_chunked.sh) now passes --no_mask to
        # generate_meta.py, so hand reconstruction works without
        # tool_masks/masks.h5 being present.
